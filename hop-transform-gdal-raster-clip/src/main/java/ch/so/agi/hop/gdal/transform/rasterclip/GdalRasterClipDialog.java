...
public class GdalRasterClipDialog extends BaseTransformDialog {
  ...
  public GdalRasterClipDialog(
      Shell parent, IVariables variables, GdalRasterClipMeta transformMeta, PipelineMeta pipelineMeta) {
  ...
  public String open() {
    ...
    RasterDialogUiSupport.TabSection clipTab =
        RasterDialogUiSupport.createTabSection(wTabFolder, "Clip options");
    ...
    tabSections = List.of(inputTab, outputTab, clipTab, remoteTab, advancedTab);

    buildInputTab(inputTab.content(), middle, margin);
    buildOutputTab(outputTab.content(), middle, margin);
    buildClipTab(clipTab.content(), middle, margin);
    ...
    wbTemplateDataset.addListener(SWT.Selection, e -> browseTemplate());
  ...
  private void populateCombos() {
    ...
    wTemplateSourceMode.setItems(inputModes);
  ...
  private void getData() {
    ...
    wAdditionalArgs.setText(Utils.isEmpty(input.getAdditionalClipArgs()) ? "" : input.getAdditionalClipArgs());
    wAddAlpha.setSelection(input.isAddAlpha());
  ...
  private void buildClipTab(Composite content, int middle, int margin) {
    ...
    last =
        row(
            ...
            middle,
            margin,
            w -> wTemplateSourceMode = (ComboVar) w,
            parent -> new ComboVar(variables, parent, SWT.SINGLE | SWT.LEFT | SWT.BORDER));
    last =
        rowWithButton(
            content,
    ...
    last =
        row(
            ...
            middle,
            margin,
            w -> wTemplateLayerName = (TextVar) w,
            parent -> new TextVar(variables, parent, SWT.SINGLE | SWT.LEFT | SWT.BORDER));
    row(
        content,
        last,
  ...
  private void ok() {
    ...
    input.setAdditionalClipArgs(wAdditionalArgs.getText());
    input.setAddAlpha(wAddAlpha.getSelection());
  ...
  private void browseTemplate() {
    ...
    String selected = dialog.open();
    if (selected != null) {
      wTemplateDatasetValue.setText(selected);
    }
  }
