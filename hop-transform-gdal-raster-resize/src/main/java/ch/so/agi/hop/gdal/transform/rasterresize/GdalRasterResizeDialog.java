...
public class GdalRasterResizeDialog extends BaseTransformDialog {
  ...
  public GdalRasterResizeDialog(
      Shell parent, IVariables variables, GdalRasterResizeMeta transformMeta, PipelineMeta pipelineMeta) {
  ...
  private void getData() {
    ...
    wAdditionalArgs.setText(Utils.isEmpty(input.getAdditionalResizeArgs()) ? "" : input.getAdditionalResizeArgs());
  ...
  private void buildAdvancedTab(Composite content, int middle, int margin) {
    ...
    last =
        row(content, last, "Creation options", middle, margin, w -> wCreationOptions = (TextVar) w,
            parent -> new TextVar(variables, parent, SWT.SINGLE | SWT.LEFT | SWT.BORDER));
    last =
        row(content, last, "Additional resize args", middle, margin, w -> wAdditionalArgs = (TextVar) w,
            parent -> new TextVar(variables, parent, SWT.SINGLE | SWT.LEFT | SWT.BORDER));
    last =
  ...
  private void ok() {
    ...
    input.setAdditionalResizeArgs(wAdditionalArgs.getText());
